...
const Console: React.FC<consoleprops> = ({Url, Status,who,close}) => {
  const lines = [
    <h1 className='text-[#5D666B]' key="A">{'C://Phishbust > '}<span className='animate-typing text-clip relative'>npm install Phishbust</span></h1>,
    <br key="B"/>,
    <p key="C">
  ...
  return (
    <div className='bg-[#12171A] z-20 mt-20 rounded-[8px] shadow-2xl w-full max-w-[80vw] lg:max-w-[40vw] flex-1 p-5'>
      ...
      {displayedLines.map((line,index) => (
        <div className="flex flex-row" key={index}>{line}</div>))}
      </div>
    </div>
  )
}
